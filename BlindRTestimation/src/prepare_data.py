...
RIR_DIR = os.path.join(ROOT, "training_rirs_16khz_1")
LABELS_CSV = os.path.join(ROOT, "ground_truth_t60_filtered.csv")
...
# Speaker split: disjoint across train/val/test (p009 does not exist in EARS-10)
SPEAKERS = {
...
def stratified_rir_split(df, rng):
    """80/10/10 split stratified by 0.1 s T60 bins."""
    split = pd.Series("train", index=df.index)
    bins = (df["T60_s"] // 0.1).astype(int)
    for _, idx in df.groupby(bins).groups.items():
        idx = rng.permutation(np.array(idx))
        n = len(idx)
        n_test = max(1, round(0.1 * n)) if n >= 3 else 0
        n_val = max(1, round(0.1 * n)) if n >= 3 else 0
