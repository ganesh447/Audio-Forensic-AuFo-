...
def fmt_title(label: str, fs: int, dur: float, peak: float, rms: float,
              spl: float | None, lag: float, coeff: float | None):
    parts = [label, f"fs {fs} Hz", f"{dur:.2f} s",
             f"peak {peak:.4g}", f"RMS {rms:.4g}"]
    if spl is not None:
        parts.append(f"{spl:.1f} dB SPL")
    parts.append(f"lag {lag:+.3f} s")
    if coeff is not None:
        parts.append(f"corr {coeff:+.3f}")
    return " — ".join(parts)
...
def main() -> None:
    ...
    traces.append({
        ...
        "unit": "Amplitude",
        "title": fmt_title("Reference (studio)", fs_ref, ref.size / fs_ref,
                           peak, rms, None, 0.0, None),
    })
    ...
    for c in range(bk.shape[1]):
        ...
        traces.append({
            ...
            "title": fmt_title(f"BK Ch {c + 1} (SN {serials[c]})", fs_bk,
                               y.size / fs_bk, peak, rms, spl, lag_bk, coeff_bk),
        })
    ...
    traces.append({
        ...
        "unit": "Amplitude",
        "title": fmt_title("RAFA iPhone bottom mic", fs_rafa, rafa.size / fs_rafa,
                           peak, rms, None, lag_rafa, coeff_rafa),
    })
    ...
    traces.append({
        ...
        "unit": "Amplitude",
        "title": fmt_title("VoIP", fs_voip, voip.size / fs_voip,
                           peak, rms, None, lag_voip, coeff_voip),
    })
    ...
    fig.suptitle("p046 emo_amazement_freeform — 7-signal comparison (aligned to reference)")
    fig.tight_layout()
