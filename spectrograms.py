...
def main():
    ...
    # --- Figure: 5 time panels + 3 spectrograms + 3 frequency plots ---
    ...
    last_spec_ax.set_xlabel("Time [s]")

    # --- Frequency domain plots (independent x-axis) ---
    spec_ax      = fig.add_subplot(gs[8])
    third_oct_ax = fig.add_subplot(gs[9])
    norm_spec_ax = fig.add_subplot(gs[10])

    plot_bk_spectrum(spec_ax, bk_pa_ch, fs_bk)
    plot_bk_third_octave(third_oct_ax, bk_pa_ch, fs_bk)
    plot_normalized_spectrum(norm_spec_ax, {
